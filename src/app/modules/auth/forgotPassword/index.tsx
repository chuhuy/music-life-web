...
const ForgotPasswordScreen: React.FunctionComponent<Props> = (props: Props) => {
  ...
  const handleForgotPassword = (email: any) => {
    if (/^.+@.+$/.test(email)) {
        ...
        .then((res) => {
            if(res.status) window.alert('We have just sent you new password. Please check your email');
            else window.alert('Cannot reset your password');
        })
        .catch((error) => {
            console.log(error);
            window.alert('Cannot reset your password')
        })
    } else {
                      ...
                      <div className="row no-gutters my-auto">
                        <div className="col-10 col-lg-9 mx-auto text-center">
                          <div className="logo mt-5 mb-3">
                            {" "}
                            {/* <a href="index.html" title="Music Life">
                          <img
